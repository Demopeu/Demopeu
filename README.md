...
- **프론트엔드 전담 1인**으로 메인, 상품 상세, 장바구니 등 핵심 페이지 전체 설계 및 개발
- **Next.js 15 + React 19 App Router 기반 SSR/CSR 혼합 구조 설계**로 초기 로딩 최적화 및 SEO 대응
- **shadcn UI + Zustand 조합으로 디자인 일관성과 상태 분리 구조 구현**
- **Optimistic UI, 태그 기반 캐싱, 전역 상태 관리 등 실제 서비스에 가까운 사용자 경험 설계**
...
- 프론트엔드 단독 1인으로 **Next.js 15 + React 19 기반의 SSR/CSR 혼합 구조를 기획하고 전면 개발**
- 메인 / 상품 상세 / 장바구니 등 **실제 커머스 핵심 흐름을 중심으로 UX 설계부터 기능 구현까지 단독 수행**
- `revalidateTag()`, `useOptimistic`, `Zustand` 등 **최신 기능을 실무 수준에서 도입**
- 협업 중 발생한 **API 응답 구조 변경, 에러 처리, 비동기 데이터 흐름 등 실시간 커뮤니케이션을 주도**하며 개발 완료
...
- **프론트엔드 전담 1인**으로 상세 페이지 UI/UX 구성 및 무한 대댓글 기능 개발
- flat 댓글 데이터를 **parent-child 트리 구조로 변환**하여 계층적 렌더링 구현
...
### 1️⃣ flat 댓글 데이터를 트리 구조로 변환

- **문제**: API는 `parent_idx`만 포함된 flat 리스트 제공
- **해결**: `parent_idx` 기반 계층화 로직 작성, `children` 배열 생성으로 트리 구조 렌더링

### 2️⃣ 댓글 작성 후 페이지 상태 유지

- **문제**: 댓글 작성 후 1페이지로 리셋되는 UX 발생
- **해결**: `localStorage`로 페이지 상태 저장 및 복원
...
- **React + Firebase 조합으로 달력 기반 팬페이지를 직접 설계하고 개발**
- `react-router-dom`을 활용해 월 단위 라우팅 처리 및 UI 상태 유지 구현
- SweetAlert2로 유튜브 영상 모달 인터랙션 구성
- **Firebase Hosting을 통한 서버리스 배포 환경 구성 및 SPA 라우팅 이슈 해결**
...
- **개인 프로젝트로 프론트엔드의 전반적인 흐름을 처음부터 끝까지 경험**
  → 기획, 컴포넌트 설계, 상태 관리, 배포까지 모두 직접 수행

- Firebase Hosting의 SPA 라우팅 문제나, 달력 UI의 마우스 이벤트 충돌 등  
  **실제 프론트엔드에서 자주 마주치는 문제를 직접 겪고 해결하며 실전 감각을 키움**

- 단순히 작동하는 기능이 아닌, 사용자 입장에서 **자연스럽게 인터랙션이 흐르도록 고민하며 구현**

- **React의 이벤트 흐름, 라우팅 처리, CSS 레이아웃 구조에 대한 이해도가 크게 향상된 계기**
...
- **Figma로 전체 UI/UX 디자인 직접 기획**, Vue.js + Bootstrap으로 반응형 웹 구현
- **나라 선택, 추천, 커뮤니티, 프로필 등 모든 페이지 UI 구현 및 사용자 인터랙션 처리**
- **Django 백엔드와의 API 연동 및 CORS 해결**, 프론트-백 데이터 흐름 구조 설계에 참여
- **slot/prop 방식의 컴포넌트화 설계**, NavBar, Selector, Card 등 재사용 가능한 구조로 구성
...
- 디자이너 없이 **UI/UX 디자인부터 기능 구현까지 단독 수행**, 인터페이스 설계 경험
- 컴포넌트화, 상태 관리, 라우팅, 백엔드 연동까지 직접 다루며 **프론트 중심의 풀스택 개발 경험 축적**
- Django API 연동 및 CORS 이슈 해결을 통해 **프론트-백 간의 통신 구조에 대한 실무 이해도 향상**
