...
**React & Next.js:** v14~16 App Router 기반 렌더링 전략 및 캐싱 전략 설계, RSC 렌더링 파이프라인 트러블슈팅 경험, React Compiler 도입 <br/>
...
- **프로젝트명:** 스타벅스 쇼핑 서비스 리뉴얼 프로젝트
- **진행 기간:** 2025.03.10 ~ 2025.04.28 (약 7주)
- **팀 구 성:** 총 5명 (프론트엔드 1, 백엔드 4)
- **담당 역할:** 프론트엔드 전담 (메인, 상품 상세, 장바구니 페이지 UI/UX 설계 및 개발)
- **GitHub:** [저장소 링크](https://github.com/Demopeu/FE-Starbucks-Renewal) _(각 페이지의 상세 구현 화면은 GitHub 저장소에서 확인하실 수 있습니다.)_
...
- **Infra:** AWS, GitHub, Notion, Slack

**문제 원인 및 해결 과정**

- **[클릭 시 데이터 패칭 지연에 따른 즉시 UI 반영 필요]**
  - **문제:** 클릭 시 서버 데이터 패칭이 완료될 때까지 UI가 반영되지 않아 사용자가 **느리게 느끼는 UX** 발생.
  - **해결:** `useOptimistic` 훅을 사용하여 **Optimistic UI** 적용 → UI는 즉시 반영하고, 백그라운드에서 서버 동기화.
- **[`useOptimistic` 사용에 따른 전역 상태 관리 어려움]**
...
- **[빠른 클릭으로 인한 API 요청 과다 및 서버 부하]**
  - **문제:** 체크박스를 연속 클릭 시 **API가 여러 번 호출**, 서버 부하 및 DB 트래픽 증가.
  - **해결:** **사용자 입력 딜레이와 요청 제어**를 위한 **커스텀 훅 개발** (클릭 시 debounce/throttle 적용, API 요청 중일 땐 추가 요청 차단, 요청 완료 후 상태 갱신).
- **[React 리렌더링 최적화 및 LCP 47% 개선]**
  - **문제:** 상태 변화마다 **모든 컴포넌트 리렌더링 발생 → 성능 저하**.
  - **해결:** `useCallback`으로 콜백 함수 메모이제이션(함수 재생성 방지), `React.memo`로 prop 동일 시 리렌더링 방지 처리.
- **[Next.js 15 캐시 전략 설계 및 API 호출 절감]**
  - **문제:** **모든 API 요청 캐시 invalidation → 필요 없는 요청까지 갱신**되는 비효율 발생.
  - **해결:** `fetch` 요청에 `next: { tags: [...] }` 옵션을 사용하여 캐시 단위를 세분화하고(예: `CartItem:uuid`, `CartUuidsList-cartType`, `Product-uuid` 등), `revalidateTag()`로 **단위별 무효화** 처리.
...
- React 리렌더링 최적화 로직을 적용하여 핵심 웹 성능 지표인 **LCP 수치를 47% 개선** 달성.
  | 지표 | 최적화 전 | 최적화 후 | 변화 |
  |----|----|----|----|
  | FCP | 0.9초 | 0.9초 | 거의 동일 |
  | LCP | 3.2초 | 1.7초 | **47% 개선 ✅** |
  | Speed Index | 1.4초 | 1.4초 | 소폭 개선 |
- 프론트엔드 단독 1인으로 **Next.js 15 + React 19 기반의 SSR/CSR 혼합 구조를 기획하고 전면 개발** 수행.
- 백엔드 4인과의 협업을 통해 **API 응답 형식, 에러 핸들링, 예외 상황 처리 방식 등을 실시간으로 조율**하며 프론트 주도적으로 방향 설정.
- 프론트 1인 구성으로 전체 흐름을 설계하며, **디자인 시스템과 상태 관리를 어떻게 통합할지에 대한 구조적 고민을 실제로 경험함**.
